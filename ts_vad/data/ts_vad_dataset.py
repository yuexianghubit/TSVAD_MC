...
class TSVADDataset(FairseqDataset):
    def __init__(
        ...
        if self.mic_idx >= 0:
            logger.info(f"Single-channel setting, mic_idx={self.mic_idx}")
        else:
            logger.info(f"Multi-channel setting, mic_idx={self.mic_idx}")
        ...
        for line in lines:
            ...
            # str_filename = filename.split("_") 
            # try:
            #     if str_filename[4] == "bound" and str_filename[3] == "chn00":
            #         self.dataset_name = "ntu"
            #         pass
            #     else:
            #         continue
            # except:
            #     print("not ntu dataset")
            labels = dict['labels']
    ...
    def __getitem__(self, index):
        ...
        ref_speech, labels, new_speaker_ids = self.load_rs(file, speaker_ids, start, stop, self.mic_idx)  # ref_speech(64240,8)
        if self.spk_path is None:
            target_speech = self.load_ts(file, speaker_ids)
        else:
            target_speech = self.load_ts_embed(file, new_speaker_ids)
        if self.dataset_name == "ntu":
            if ref_speech.shape[1] == 3:
                ref_speech = torch.cat((ref_speech, ref_speech, ref_speech[:,:2]), dim=1)
            elif ref_speech.shape[1] == 4:
                ref_speech = torch.cat((ref_speech, ref_speech), dim=1)
            elif ref_speech.shape[1] == 2:
                ref_speech = torch.cat((ref_speech, ref_speech, ref_speech, ref_speech), dim=1)
        else:
            ref_speech = ref_speech if ref_speech.ndim==2 else ref_speech.unsqueeze(1).repeat(1, 8)
        sample = {
            'id': index,
    ...
    def load_rs(self, file, speaker_ids, start, stop, mic_idx):
        ref_speech, _ = sf.read(os.path.join(self.audio_path, file + '/all.wav'), start=start * 640,
                                stop=stop * 640 + 240)  # Since 25 * 640 = 16000

        # mic_idx > 0, it is the single-channel setting
        if mic_idx >= 0:
            ref_speech = ref_speech[:, mic_idx]
        ...
        max_length = max(len(label) for label in labels)  # 找到最大长度
        padded_labels = [np.pad(label, (0, max_length - len(label)), 'constant') for label in labels]
        labels = np.array(padded_labels)
        labels = torch.from_numpy(labels).float()  # 4, T
    ...
    def load_ts_embed(self, file, speaker_ids):
        ...
        for speaker_id in speaker_ids:
            if speaker_id == -1:
                if self.shuffle_spk_embed_level == 3 and (
                        np.random.choice(2, p=[1 - self.zero_ratio, self.zero_ratio]) == 1 or not self.is_train):
    ...
    def load_ts(self, file, speaker_ids):
        ...
        for speaker_id in speaker_ids:
            ...
            target_speech = kaldi.fbank(target_speech, num_mel_bins=80, frame_length=25, frame_shift=10, dither=1.0,
