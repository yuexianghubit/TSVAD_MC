...
@register_task("ts_vad_mc", dataclass=TSVADTaskConfig)
class TSVAD_MC_Task(FairseqTask):
    """
        ...
        if self.cfg.task_type == "diarization":
            if 'train' in split.lower() and self.cfg.shuffle_spk_embed_level != 0:
                ...
                if os.path.isdir(f"{self.cfg.spk_path}/{split}/ecapa_avg_feature_dir"):
                    ...
                    spk_path = f"{self.cfg.spk_path}/{split}/ecapa_feature_dir"
                if self.cfg.dataset_name == "ntu":
                    spk_path = f"{self.cfg.spk_path}/ecapa_feature_dir"

            
            if self.cfg.dataset_name == "alimeeting":
                json_path = f"{self.cfg.data}/{split}_Ali/{split}_Ali_far/{split}.json"
                audio_path = f"{self.cfg.data}/{split}_Ali/{split}_Ali_far/target_audio_mc"
            if self.cfg.dataset_name == "alimeeting_ami" or self.cfg.dataset_name == "alimeeting_ami_chime5":
                if 'train' in split.lower():
                    json_path = f"{self.cfg.data}/{split}/{split}.json"
                    audio_path = f"{self.cfg.data}/{split}/target_audio_mc"
                else:
                    json_path = f"{self.cfg.data}/{split}_Ali/{split}_Ali_far/{split}.json"
                    audio_path = f"{self.cfg.data}/{split}_Ali/{split}_Ali_far/target_audio_mc"
            if self.cfg.dataset_name == "ntu":
                json_path = f"{self.cfg.data}/eval_bound.json"
                audio_path = f"{self.cfg.data}/target_audio_bound_mc"
