...
def get_args():
    ...
    parser.add_argument('--data_path', required=True, help='the path for the root')
    parser.add_argument('--orig_audio_path', required=True, help='the path for the orig audio')
    parser.add_argument('--target_audio', required=True, help='the part for the output audio')
    parser.add_argument('--target_embedding', required=True, help='the part for the output audio')
    parser.add_argument('--rttm_path', required=True, help='the path for the rttm_files')
    parser.add_argument('--max_speaker', default=8, type=int, help='max number of speakers')
    parser.add_argument('--type', default='Train', help='can pass this for custom file name')
    parser.add_argument('--source', required=True, help='the part for the speaker encoder')
    ...
    os.makedirs(args.target_audio, exist_ok = True)
    os.makedirs(args.target_embedding, exist_ok = True)
...
def main():
	...
	rttms = glob.glob(args.rttm_path + '/*.rttm')
	...
	for rttm in tqdm.tqdm(rttms):
		...
		# wav_file = glob.glob(os.path.join(args.orig_audio_path, uttid) + '*.wav')[0]
		wav_file = os.path.join(args.orig_audio_path, uttid + '.wav')
		...
		for key in new_intervals:
			output_dir = os.path.join(args.target_audio, id_full)
	...
	# Extract embeddings
	files = sorted(glob.glob(args.target_audio + "/*/*.wav"))
	...
	for file in tqdm.tqdm(files):
		if 'all.wa' not in file:
			...
			for start in range(0, wav_length - int(args.length_embedding * 16000), int(args.step_embedding * 16000)):
				...
				# target_speech = numpy.mean(target_speech, axis=-1)
